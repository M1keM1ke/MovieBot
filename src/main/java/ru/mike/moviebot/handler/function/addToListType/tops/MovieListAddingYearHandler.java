package ru.mike.moviebot.handler.function.addToListType.tops;
...
@Component
public class MovieListAddingYearHandler
    ...
    public String getMessage(Update update, String userMessage) {
        ...
        if (list.isPresent() && (userByChatId.getUserState() == BotState.CURRENT_YEAR_TOP ||
        userByChatId.getUserState() == BotState.CONTINUE_YEAR)) {
    ...
    public void execute(BotApplication botApplication, Update update) {
        ...
        if (!inlineKeyboardMarkup.isPresent()) {
            ...
            InlineKeyboardMarkup continueYearButton = keyboardUtils
                            .continueYearTopKeyboard().setInlineContinueByYearAndAddToListTopButtons();
            editMessageCaption.setReplyMarkup(continueYearButton);
        }
